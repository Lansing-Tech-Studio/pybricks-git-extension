...
Phase 1 (template repo v2) is done in the starter repo; the manifest and menu-config contract committed there are the interfaces this extension codes against. Remaining phases, in order, all in this repo:

- **Phase 2 — protected files**: engine reads `.pybricks-git.json` from the fetched remote head; `commitOp` keeps the tree version of protected paths and reports a `protectedSkipped` list; `pullOp` returns the protected set; `content.js` shows a notice. Small; do first.
